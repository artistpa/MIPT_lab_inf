...
![graph](1/img/insert_comparison_size_from_100_to_15000.png)

Из графика видно, что для стандартного вектора асмптотика O(N) (угол наклона прямой больше 0, хорошо видно при приближении к size = 1500). Для subvector время работы значительно увеличивается с ростом size по сравнению с std::vector. Асимптотика для subvector - O(N) (из кода и графика).
